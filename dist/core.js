...
var repeat = function (times, block) {
    if (times < 0) {
        error(new Exceptions_1.IllegalArgumentException("Error while calling 'repeat'. Parameter 'times' must be greater or equal '0'."));
    }
...
var repeatDownTo = function (from, to, block) {
    if (from < to) {
        error(new Exceptions_1.IllegalArgumentException("Error while calling 'repeatDownTo'. Parameter 'from' must be greater or equal to parameter 'to'."));
    }
    var index = from;
    while (index >= to) {
        block(index);
...
var repeatUpTo = function (from, to, block) {
    if (from > to) {
        error(new Exceptions_1.IllegalArgumentException("Error while calling 'repeatUpTo'. Parameter 'from' must be smaller or equal to parameter 'to'."));
    }
